...
export function MultiSelectCombobox({
  ...
  const showCompactCount =
    compactCountOnMulti && normalizedSelectedValues.length > 1;
  const compactPrimaryValue = normalizedSelectedValues[0] ?? "";
  const compactRemainingCount = Math.max(
    0,
    normalizedSelectedValues.length - 1,
  );
  const isCompactPrimaryLocked =
    normalizedDefaultValues.includes(compactPrimaryValue);
            ...
            <>
              {visibleChips.map((value) => {
                ...
                return (
                  <div
                    key={value}
                    className="flex items-center gap-1 bg-gray-100 border border-gray-300 text-gray-700 px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap"
                  >
                    <span className="truncate max-w-[50px]">{value}</span>
                    {!isLocked && (
                      <div
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className="hover:bg-gray-200 rounded-full p-0 inline-flex items-center justify-center flex-shrink-0 transition-colors cursor-pointer"
                        role="button"
                        tabIndex={0}
              ...
              {showCountBadge && remainingCount > 0 && (
                <div
                  ...
                  className="flex items-center gap-1 bg-blue-100 border border-blue-300 text-blue-700 px-2 py-0.5 rounded-full text-xs font-medium hover:bg-blue-200 transition-colors whitespace-nowrap cursor-pointer"
                  role="button"
                  tabIndex={0}
          ...
          <div className="max-h-48 overflow-y-auto p-2">
            {normalizedSelectedValues.map((value) => {
              ...
              return (
                <div
                  key={value}
                  className="flex items-center justify-between gap-2 bg-gray-50 border border-gray-200 text-gray-700 px-2 py-1.5 rounded text-sm mb-1 last:mb-0"
                >
